...
// Como refVal, mas OMITE o <c:pt> dos pontos null: com dispBlanksAs=gap a linha
// se quebra naquela categoria em vez de descer a zero. O ptCount continua sendo
// o total de categorias (o índice ausente é que vira a lacuna).
function refValGap(vals, col) {
  const pts = vals
    .map((v, i) =>
      v == null || !Number.isFinite(Number(v))
        ? ''
        : `<c:pt idx="${i}"><c:v>${Number(v).toFixed(4)}</c:v></c:pt>`
    )
    .join('')
  return (
    `<c:val><c:numRef><c:f>Plan1!$${col}$2:$${col}$${vals.length + 1}</c:f><c:numCache>` +
    '<c:formatCode>#,##0.00</c:formatCode>' +
    `<c:ptCount val="${vals.length}"/>${pts}</c:numCache></c:numRef></c:val>`
  )
}
...
// ------------------------------------------------------------- linhas ---
// Gráfico de LINHAS nativo (c:lineChart): o eixo x são as categorias (as fases
// do ciclo) e cada série é uma linha (um exercício). Sem rótulo por ponto — o
// eixo de valor fica VISÍVEL, com grade, para dar a escala sem poluir 25 marcas.
// Pontos nulos viram lacuna (refValGap + dispBlanksAs="gap"): a linha se quebra
// na fase ausente em vez de desabar a zero.
//
// Ordem dos elementos é obrigatória no OOXML (fonte de "arquivo precisa de
// reparo"): CT_LineSer = idx, order, tx, spPr, marker, cat, val, smooth;
// CT_LineChart = grouping, varyColors, ser*, marker, axId, axId.
function graficoLinhas({ cats, series, formato = FMT_BI, legenda = true, yMin, yMax }) {
  cats = Array.isArray(cats) ? cats.slice() : []
  series = Array.isArray(series) ? series.map((s) => ({ ...s })) : []
  if (cats.length === 0 || series.length === 0) {
    cats = ['(sem dados no recorte selecionado)']
  ...
  // Alinha cada série ao nº de categorias, preservando o null (vira lacuna).
  series = series.map((s) => ({
    ...s,
    valores: cats.map((_, j) => {
      const v = s.valores?.[j]
  ...
  const sers = series
    .map((s, i) => {
      const col = String.fromCharCode(66 + i) // B, C, …
      const hex = String(s.cor ?? '').replace('#', '')
      const cor = /^[0-9a-f]{6}$/i.test(hex) ? hex.toUpperCase() : '2A78D6'
      return (
        `<c:ser><c:idx val="${i}"/><c:order val="${i}"/>` + refNome(s.nome, col) +
        `<c:spPr><a:ln w="28575" cap="rnd"><a:solidFill><a:srgbClr val="${cor}"/></a:solidFill>` +
        '<a:round/></a:ln></c:spPr>' +
        '<c:marker><c:symbol val="circle"/><c:size val="6"/><c:spPr>' +
        `<a:solidFill><a:srgbClr val="${cor}"/></a:solidFill>` +
        '<a:ln w="9525"><a:solidFill><a:srgbClr val="FFFFFF"/></a:solidFill></a:ln>' +
        '</c:spPr></c:marker>' +
        refCat(cats) + refValGap(s.valores, col) + '<c:smooth val="0"/></c:ser>'
      )
    })
    .join('')
  ...
  // CT_Scaling exige a ordem logBase, orientation, max, min. Quando yMin/yMax
  // vêm definidos, o eixo dá o mesmo "zoom" da tela (faixa dos dados ± margem)
  // em vez de começar no zero.
  const escala =
    '<c:scaling><c:orientation val="minMax"/>' +
    (Number.isFinite(yMax) ? `<c:max val="${yMax}"/>` : '') +
    (Number.isFinite(yMin) ? `<c:min val="${yMin}"/>` : '') +
    '</c:scaling>'
  const valAx =
    `<c:valAx><c:axId val="${ax2}"/>` + escala +
    '<c:delete val="0"/><c:axPos val="l"/>' +
    `<c:majorGridlines><c:spPr><a:ln w="9525"><a:solidFill><a:srgbClr val="${BORDA}"/></a:solidFill></a:ln></c:spPr></c:majorGridlines>` +
    `<c:numFmt formatCode="${formato}" sourceLinked="0"/>` +
    '<c:majorTickMark val="none"/><c:minorTickMark val="none"/><c:tickLblPos val="nextTo"/>' +
    txPr(1000, FRACA) +
...
// Mesma ordem de FASES_PPTX — ids usados por `fasesVazias` (ano → ids de fase
// sem coluna própria na planilha). Espelha FASE_IDS de ploa.js.
const FASE_IDS_PPTX = ['pl', 'setorial', 'geral', 'plenario', 'autografo']

// Trajetória de um exercício ao longo das fases, em R$ bilhões. As fases sem
// coluna na planilha daquele ano (semAut) saem como null → lacuna na linha.
function valoresCicloAno(a, fasesVazias = {}) {
  const vazias = new Set((fasesVazias && fasesVazias[a.ano]) || [])
  return FASE_IDS_PPTX.map((id, i) => (vazias.has(id) ? null : bi(a.fases?.[i] ?? 0)))
}
// Uma cor por exercício, na MESMA ordem da tela (CORES_ANO): a linha do ano i
// no slide tem a cor do cartão do ano i.
const CORES_ANO_PPTX = [AZUL, VERDE, LARANJA, VIOLETA, AQUA, MAGENTA, AMARELO, VERMELHO]
...
function paineisHistoricoPLOA(d) {
  ...
  // Zoom do eixo do ciclo (em bilhões): 1 bi abaixo do menor ponto e 1 bi acima
  // do maior, espelhando `margemEixo` da tela. Achata o eixo na faixa dos dados.
  const ptsCiclo = d.resumoAnos.flatMap((a) => valoresCicloAno(a, d.fasesVazias).filter((v) => v != null))
  const cicloYMin = ptsCiclo.length ? Math.max(0, Math.floor(Math.min(...ptsCiclo) - 1)) : undefined
  const cicloYMax = ptsCiclo.length ? Math.ceil(Math.max(...ptsCiclo) + 1) : undefined
  const linhaMatriz = (l) => {
    const celulas = l.valores.map((v, i) => {
      const anterior = i > 0 ? l.valores[i - 1] || 0 : 0
      const subiu = i > 0 && anterior > 0 && v > anterior
  ...
  return [
    ...
    },
    {
      ...
      sub: 'Uma linha por exercício ao longo das fases do rito — PL → Autógrafo',
      total: fmtBiTxt(d.totalPeriodo),
      grafico: graficoLinhas({
        cats: FASES_PPTX,
        series: d.resumoAnos.map((a, i) => ({
          nome: String(a.ano),
          cor: CORES_ANO_PPTX[i % CORES_ANO_PPTX.length],
          valores: valoresCicloAno(a, d.fasesVazias),
        })),
        formato: FMT_BI,
        legenda: true,
        yMin: cicloYMin,
      ...
      planilha: planilha(FASES_PPTX, d.resumoAnos.map((a) => ({
        nome: String(a.ano),
        valores: valoresCicloAno(a, d.fasesVazias),
      }))),
    },
