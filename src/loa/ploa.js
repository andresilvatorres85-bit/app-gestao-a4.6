...
// Ciclo como LINHAS: a transposta de `ciclosPorAno`. Em vez de uma série por
// fase (com o ano no eixo), devolve uma série por EXERCÍCIO — cada ano é uma
// linha percorrendo as cinco fases do rito, do PL ao autógrafo. É a leitura da
// TRAJETÓRIA de cada exercício ao longo do ciclo.
//
// Aqui o ano vai deliberadamente na COR (uma linha por exercício), o que é a
// exceção pedida à regra geral do app ("ano é ordinal, vai na posição"): numa
// leitura de trajetória a posição já é a FASE, então a série só pode ser o ano.
// A cor não é uma rampa sequencial (que reprovaria no ΔE), e sim a paleta
// categórica já validada — cada linha também se distingue pelo próprio traçado.
//
// `fasesVazias` (ano → ids de fase sem coluna própria na planilha) marca os
// pontos ausentes como `null`, PONTO A PONTO por exercício: a linha se quebra
// naquela fase em vez de desabar a zero. No início do rito, o exercício
// corrente só tem o PL, então sua linha é um único ponto — os demais anos
// seguem inteiros.
export function ciclosLinhasPorAno(registros, fasesVazias = {}) {
  const anos = anosPLOA(registros)
  return anos.map((ano) => {
    const doAno = registros.filter((r) => r.ano === ano)
    const fases = somaFases(doAno)
    const vazias = new Set(fasesVazias?.[ano] || [])
    const valores = FASES.map((f, i) => (vazias.has(f.id) ? null : fases[i]))
    return {
      chave: String(ano),
      rotulo: String(ano),
      ano,
