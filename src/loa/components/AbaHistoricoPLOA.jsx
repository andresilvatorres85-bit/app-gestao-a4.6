...
import {
  ...
  resumoPorAno, agregadoPorAno, uoPorAno, rpPorAno, gndPorAno, acaoPorAno,
  ciclosLinhasPorAno,
...
import GraficoLinhasFases from './GraficoLinhasFases.jsx'
...
export default function AbaHistoricoPLOA({
  ...
  const cicloLinhas = useMemo(
    () => ciclosLinhasPorAno(registros, fasesVazias),
    [registros, fasesVazias]
  )
  ...
  // Ciclo em linhas: uma linha por exercício, percorrendo as cinco fases. A cor
  // é a mesma paleta categórica validada usada nos cartões (`coresPorAno`), na
  // ordem dos exercícios — não é rampa sequencial.
  const serieCicloLinhas = cicloLinhas.map((s, i) => ({
    ...s,
  ...
  return (
    <>
      ...
      <p className="historico-intro">
        Comparativo dos {anos.length} exercícios selecionados na planilha de elaboração
        ({anos.join(', ')}). Esta subaba <strong>responde ao filtro de Ano</strong> — todos
        marcados ao entrar; desmarque exercícios para retirá-los da comparação — e respeita os
        demais filtros da barra acima. Os valores são os do
      ...
      <div className="paineis">
        ...
        {/* 6 — ciclo por exercício (linhas: uma por exercício ao longo das fases) */}
        <section className="painel-grafico p-12">
          <div className="painel-cab">
            <div className="painel-cab-txt">
              <h2>Fases do ciclo por exercício</h2>
              <p className="painel-sub">
                Uma linha por exercício ao longo das fases do rito — {FASE_ROTULOS.join(' → ')}
              </p>
            </div>
            <span className="painel-total">{fmtBi(totalPeriodo)}</span>
          ...
          <GraficoLinhasFases
            fases={FASE_ROTULOS}
            series={serieCicloLinhas}
            formatar={fmtBi}
            formatarTick={fmtBiSeco}
            margemEixo={1e9}
            rotuloEixo="Valor de cada exercício ao longo das fases do ciclo"
          />
        </section>
      ...
      <div className="folha-pdf" aria-hidden>
        <FolhaHistoricoPLOA
          ...
          seriePL={seriePL}
          serieRito={serieRito}
          serieCicloLinhas={serieCicloLinhas}
          forcas={forcas}
          uos={uos}
          rps={rps}
          gnds={gnds}
